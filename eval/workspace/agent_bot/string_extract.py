...
from config import PROJECT_DIR, PROJECT_NAME, GHIDRA_PATH

# FIXED: Removed the extra .parent and forced absolute path resolution
GHIDRA_INSTALL = Path(GHIDRA_PATH).resolve().parent.parent
...
def extract_triage_data(binary_path, output_json):
    ...
    # USE THE DYNAMIC PROJECT DIR FROM CONFIG
    project_dir = Path(PROJECT_DIR)
    
    print(f"[*] Opening existing project at: {project_dir} (Name: {PROJECT_NAME})")
    
    # USE THE DYNAMIC PROJECT NAME FROM CONFIG
...
if __name__ == "__main__":
    from config import TARGET_BINARY, STRINGS_JSON

    if not Path(TARGET_BINARY).exists():
        print(f"Error: File {TARGET_BINARY} not found.")
    else:
