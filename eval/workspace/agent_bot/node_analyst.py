...
def analyst_node(state: REState):
    ...
    # FIXED: Sort symbols to apply longest replacements first to avoid partial-word matches
    sorted_symbols = sorted(state["symbol_table"].items(), key=lambda x: len(x[0]), reverse=True)
    
    for old_key, new_val in sorted_symbols:
        # FIXED: Local scoped variable replacements
        if '@' in old_key:
            old_var, owning_func = old_key.split('@', 1)
            # Only apply variable renames if we are actively analyzing the function that owns them
            if owning_func == target:
                if re.match(placeholder_patterns, old_var) and not re.match(placeholder_patterns, new_val):
                    resolved_vars.append(f"{old_var} -> {new_val}")
                code = re.sub(rf'\b{old_var}\b', new_val, code)
        else:
            # Global replacements (like function names)
            if re.match(placeholder_patterns, old_key) and not re.match(placeholder_patterns, new_val):
                resolved_vars.append(f"{old_key} -> {new_val}")
            code = re.sub(rf'\b{old_key}\b', new_val, code)
...
"""
    ...
    try:
        ...
        for old_val_in_code, new_val_suggested in renames:
            ...
            # FIXED: Scope variables to their specific function to avoid global collisions
            if old_name.startswith("FUN_"):
                target_key = old_name
            else:
                target_key = f"{old_name}@{target}"
            ...
            if not re.match(placeholder_patterns, new_name):
                if target_key not in new_symbols or new_symbols[target_key] != new_name:
                    ...
                    tqdm.write(f"[*] Symbol Updated: {old_name} (in {target}) -> {new_name}")
